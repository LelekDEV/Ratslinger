...
func _physics_process(_delta: float) -> void:
	...
	# Honestly, I have no idea why it offsets and get's so much worse on lower scales.
	# Brute force it is
	var combat_offset_x: float = 0
	match parent.scale_float:
		3.0: combat_offset_x = 25
		2.0: combat_offset_x = 45
		1.0: combat_offset_x = 65
		_: combat_offset_x = 0
	
	parent.accuracy_bar.global_position.x = get_viewport().get_visible_rect().size.x - 85 + combat_offset_x
	parent.bullet_bar.global_position.x = get_viewport().get_visible_rect().size.x - 85 * parent.scale_float - 86 + combat_offset_x
